...
// Move native struct outside managed class
struct ClientMeta {
    int id;
    std::string name;
    SOCKET sock;
...
private:
    ...
    // Networking (Native objects managed via pointers)
    SOCKET serverSocket;
...
public:
    ServerForm() {
        ...
        // Correctly initialize native objects on native heap
...
private:
    void InitializeComponent() {
        ...
        lblClientCount = gcnew Label();
        lblClientCount->Text = "Người dùng online (0):";
        lblClientCount->Location = Point(655, 100);
        lblClientCount->AutoSize = true;
        lblClientCount->ForeColor = Color::FromArgb(0, 150, 250);
        this->Controls->Add(lblClientCount);
    ...
    void UpdateClientList() {
        ...
        for each (auto pair in clientNames) {
            lbClients->Items->Add(pair.Value);
        }
        lblClientCount->Text = "Người dùng online (" + clientNames->Count + "):";
    }
    ...
    void StartServer() {
        ...
        AddLog("Server đã khởi động thành công.");
    }
    ...
    void DiscoveryLoop() {
        ...
        while (isRunning) {
            ...
            if (b > 0) {
                buf[b] = '\0';
                if (std::string(buf) == "CHAT_DISCOVER_REQUEST") {
                    sendto(discoverySocket, "CHAT_DISCOVER_RESPONSE", (int)strlen("CHAT_DISCOVER_RESPONSE"), 0, (sockaddr*)&caddr, clen);
                }
            }
        }
    }
    ...
    void HandleClient(Object^ socketObj) {
        ...
        try {
            while (isRunning) {
                ...
                if (rawStr->StartsWith("AUTH|")) {
                    ...
                    String^ welcome = "SYSTEM|Chào mừng " + name + " đã tham gia!";
                    Broadcast(welcome, INVALID_SOCKET);
                    continue;
                }

                if (rawStr->StartsWith("MSG|")) {
                    String^ content = rawStr->Substring(4);
                    AddLog("MSG from " + myName + ": " + content);
                    String^ forward = "FROM|" + myName + "|" + content;
                    Broadcast(forward, s);
                }
            }
        } catch (Exception^ ex) {
            AddLog("Lỗi client thread: " + ex->Message);
        }
        ...
        if (myId > 0) {
            ...
            UpdateClientList();
            String^ leave = "SYSTEM|" + myName + " đã thoát.";
    ...
    void Broadcast(String^ msg, SOCKET excluded) {
        ...
        for (auto const& [sock, id] : *clientMap) {
            if (sock != excluded) {
                send(sock, (const char*)p, bytes->Length, 0);
            }
        }
    }
